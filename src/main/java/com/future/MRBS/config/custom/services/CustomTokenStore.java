package com.future.MRBS.config.custom.services;

import com.future.MRBS.config.custom.entities.MongoAccessToken;
import com.future.MRBS.config.custom.entities.MongoRefreshToken;
...
@Service public class CustomTokenStore implements TokenStore {
    ...
    @Override public OAuth2AccessToken getAccessToken(OAuth2Authentication authentication) {
        ...
        if (mongoAccessToken != null) {
            ...
            if (accessToken != null && !authenticationId.equals(
    ...
    public String getTokenValueByUserName(String userName) {
        Query query = new Query();
        query.addCriteria(Criteria.where(MongoAccessToken.USERNAME).is(userName));
        MongoAccessToken accessTokens = mongoTemplate.findOne(query, MongoAccessToken.class);
        return accessTokens != null ? accessTokens.getToken().getValue() : "";
    }
